...
if($valido && $inserimento){
	if(isset($esiste))
		...
		addSCore($score,$versione,$username,$password);
}
...
function checkScoreExistance($username, $password){
	$queryCerca = "SELECT punteggi.id FROM season, utenti, punteggi WHERE punteggi.id_utenteFK=utenti.id and punteggi.id_seasonFK=season.id and utenti.username=? and password=? and id_seasonFK=2";
	if($stmt = $GLOBALS['connessione']->prepare($queryCerca))
	{
...
function updateScore($score,$versione,$username,$password,$esiste){
	$platform = $GLOBALS['user']->piattaforma;
	$queryInsert = "UPDATE punteggi SET valore=?, version_fk=(SELECT version.id FROM version WHERE version.versione = ? AND version.piattaforma = '$platform'), data=now() WHERE id_utenteFK=(SELECT id FROM utenti WHERE username= ? and password= ?) AND id = ?;";
	
	if($stmt = $GLOBALS['connessione']->prepare($queryInsert))
	{
		$stmt->bind_param('isssi', $score,$versione,$username,$password,$esiste);
		$stmt->execute();
	}
}
...
function addSCore($score,$versione,$username,$password){
	$platform = $GLOBALS['user']->piattaforma;
	$queryInsert = "INSERT INTO punteggi (valore, version_fk, id_utenteFK,id_seasonFK,data) VALUES (?, (SELECT version.id FROM version WHERE version.versione = ? AND version.piattaforma = '$platform'), (SELECT id FROM utenti WHERE username= ? and password= ?),2,now());";
