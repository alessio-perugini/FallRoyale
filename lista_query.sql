...
CREATE TABLE info_devices(
    deviceUniqueIdentifier VARCHAR(256) PRIMARY KEY NOT NULL,
    data_reg DATETIME,
    deviceName VARCHAR(256),
    deviceType VARCHAR(256),
    deviceModel VARCHAR(256),
    graphicsDeviceID VARCHAR(256),
    graphicsDeviceName VARCHAR(256),
    graphicsDeviceVendor VARCHAR(256),
    graphicsDeviceVendorID VARCHAR(256),
    graphicsDeviceVersion VARCHAR(256),
    graphicsMemorySize VARCHAR(256),
    operatingSystem VARCHAR(256),
    operatingSystemFamily VARCHAR(256),
    processorCount VARCHAR(256),
    processorFrequency VARCHAR(256),
    processorType VARCHAR(256),
    supportedRenderTargetCount VARCHAR(256)
)
...
CREATE TABLE inviti(
    ...
    CONSTRAINT fk_mittente FOREIGN KEY (mittente) REFERENCES utenti(id) ON DELETE CASCADE ON UPDATE CASCADE,
	CONSTRAINT fk_destinatario FOREIGN KEY (destinatario) REFERENCES utenti(id) ON DELETE CASCADE ON UPDATE CASCADE
);
